...
const [serviceData] = await connection.query(`SELECT * FROM service_content WHERE status = 1 `);
...
const getalldetails = async (req, res) => {
  try {
    ...
    const [rows] = await connection.query(
     ...
     const [services] = await connection.query("SELECT * FROM services WHERE status = 1");
