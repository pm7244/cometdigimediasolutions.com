...
const CreateBlog = () => {
  ...
  useEffect(() => {
    if (values.blog_name && !values.blog_slug) {
      setValues(prev => ({ ...prev, blog_slug: generateSlug(values.blog_name) }));
    }
  }, [values.blog_name]);
  ...
  useEffect(() => {
    setValues(prev => ({ ...prev, related_blog: selectedRelatedBlogs.map(blog => blog.blog_id) }));
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_CMS_URL}api/createblog`, {
        method: "POST",
      ...
      if (result.status) {
        toast.success("Blog created successfully with content");
  ...
  return managerOpener === 1 ? (
    ...
    <div>
      ...
      <PageHeader currentpage="Create Blog" activepage="Pages" mainpage="Web Blog" />
