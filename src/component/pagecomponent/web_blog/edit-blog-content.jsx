...
const EditBlogContent = () => {
  ...
  // Fetch blog content data
  const fetchBlogContent = () => {
    fetch(`${import.meta.env.VITE_CMS_URL}api/getblogcontentbyid/${id}`)
      .then((res) => res.json())
      .then((data) => {
        if (data.status && data.data) {
          const contentData = data.data;
          setValues({
            blog_id: contentData.blog_id || "",
          ...
          if (contentData.image) {
            try {
              const parsedImages = JSON.parse(contentData.image);
          ...
          if (contentData.video) {
            try {
              const parsedVideos = JSON.parse(contentData.video);
          ...
          toast.error("Blog content not found");
          navigate("/cms/pages/web_blog/content");
        }
        setLoading(false);
      })
      .catch((err) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_CMS_URL}api/updateblogcontent/${id}`, {
        method: "PUT",
  ...
  if (loading) {
    return (
      <div>
        <PageHeader currentpage="Edit Blog Content" activepage="Pages" mainpage="Web Blog" />
